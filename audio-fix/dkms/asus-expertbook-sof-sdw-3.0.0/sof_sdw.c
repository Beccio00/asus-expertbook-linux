...
/*
 * The B9406CAA firmware describes an RT722 on SoundWire link 3 even though
 * that peripheral is not fitted.  The SoundWire core keeps the firmware-only
 * device around with SDW_SLAVE_UNATTACHED status.  soc_sdw_utils currently
 * accepts its endpoint, so sof_sdw tries to create the same SimpleJack DAI
 * link for both the real CS42L43 and the ghost RT722.  The duplicate sysfs
 * link aborts card registration with -EEXIST/-ENOMEM.
 *
 * Keep the workaround deliberately narrow: one DMI board, one codec ID and
 * only a peripheral which the SoundWire core has positively marked as
 * unattached.  A real RT722, or any other ASUS model, is never filtered.
 */
static const struct dmi_system_id b9406caa_dmi_table[] = {
	{
		.matches = {
			DMI_MATCH(DMI_SYS_VENDOR, "ASUS"),
			DMI_EXACT_MATCH(DMI_BOARD_NAME, "B9406CAA"),
		},
	},
	{}
};

static bool b9406caa_is_unattached_rt722(struct device *dev,
					 struct asoc_sdw_endpoint *sof_end)
{
	...
	sdw_dev = bus_find_device_by_name(&sdw_bus_type, NULL, sof_end->codec_name);
	if (!sdw_dev)
		return false;

	/* codec_name is resolved on sdw_bus_type and always names a slave. */
	slave = dev_to_sdw_dev(sdw_dev);
	ghost = slave->id.mfg_id == 0x025d &&
		slave->id.part_id == 0x0722 &&
		slave->status == SDW_SLAVE_UNATTACHED;

	if (ghost)
		dev_info(dev, "B9406CAA: ignoring unattached firmware ghost %s\n",
			 sof_end->codec_name);
...
static int b9406caa_prune_ghost_rt722(struct device *dev,
				      struct asoc_sdw_dailink *sof_dais,
				      int sdw_be_num, int *num_confs)
{
	...
	if (!dmi_check_system(b9406caa_dmi_table))
		return sdw_be_num;

	for (sof_dai = sof_dais; sof_dai->initialised; sof_dai++) {
		struct asoc_sdw_endpoint *sof_end, *tmp;

		for_each_pcm_streams(stream)
			old_dai_num += !!sof_dai->num_devs[stream];

		list_for_each_entry_safe(sof_end, tmp, &sof_dai->endpoints, list) {
			if (!b9406caa_is_unattached_rt722(dev, sof_end))
				continue;

			list_del_init(&sof_end->list);
			removed++;
			if (*num_confs > 0)
				(*num_confs)--;
		}
		...
		for_each_pcm_streams(stream) {
			sof_dai->num_devs[stream] = 0;
			sof_dai->link_mask[stream] = 0;
		}
		...
		list_for_each_entry(sof_end, &sof_dai->endpoints, list) {
			for_each_pcm_streams(stream) {
				...
				sof_dai->link_mask[stream] |= sof_end->link_mask;
			}
		}

		for_each_pcm_streams(stream)
			new_dai_num += !!sof_dai->num_devs[stream];
	}

	if (!removed)
		return sdw_be_num;

	return sdw_be_num - old_dai_num + new_dai_num;
}
			       ...
			       struct snd_soc_codec_conf **codec_conf)
{
	...
	while (sof_dais->initialised) {
		...
		if (list_empty(&sof_dais->endpoints)) {
			sof_dais++;
			continue;
		}
...
static int sof_card_dai_links_create(struct snd_soc_card *card)
{
	...
	sdw_be_num = b9406caa_prune_ghost_rt722(dev, sof_dais, ret, &num_confs);
